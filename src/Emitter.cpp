...
void Emitter::Init(unsigned int a_max_particles, vec4 a_position, float a_emit_rate, float a_min_lifespan, float a_max_lifespan,
	float a_min_velocity, float a_max_velocity, float a_start_size, float a_end_size, vec4 a_start_color, vec4 a_end_color)
{
...
void Emitter::EmitParticles()
{
	...
	for (unsigned int i = 0; i < particles_to_emit && m_alive_count < m_max_particles; ++i)
	{
		...
		m_particles[m_alive_count].position = sin(m_position);
		...
		m_particles[m_alive_count].velocity.z = 0;
		//sin(m_particles[m_alive_count].position.x);
		//cos(m_particles[m_alive_count].position.y);
		++m_alive_count;
...
void Emitter::Update(float a_dt)
{
	...
	//incerement timer and emit particles
	m_emit_timer += a_dt;
	EmitParticles();
...
void Emitter::UpdateVertexData(vec3 a_camera_pos, vec3 a_camera_up)
{
	for (unsigned int i = 0; i < m_alive_count; ++i)
	{
		...
		vec3 to = a_camera_pos - m_particles[i].position.xyz;
		vec3 f = glm::normalize(to);
		vec3 r = cross(a_camera_up, f);
		...
		m_vertex_data[i * 4 + 0].position = m_particles[i].position = particle_transform * vec4(-1, 1, 0, 1);
		m_vertex_data[i * 4 + 1].position = m_particles[i].position = particle_transform * vec4(-1, -1, 0, 1);
		m_vertex_data[i * 4 + 2].position = m_particles[i].position = particle_transform * vec4(1, -1, 0, 1);
		m_vertex_data[i * 4 + 3].position = m_particles[i].position = particle_transform * vec4(1, 1, 0, 1);
